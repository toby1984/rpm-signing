...
| `--sign-server` | Base URL of a signing server | `--sign-server http://signer:8080` | Signs the RPM remotely: the header (everything up to the payload) is POSTed to `<base url>/sign` and the response is merged with the local payload into the output file. Only valid together with `-s`/`--sign`, and mutually exclusive with `--priv-key` and `--priv-key-password`. |
